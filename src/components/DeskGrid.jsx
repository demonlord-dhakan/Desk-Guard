...
export default function DeskGrid({ desks, selectedDeskId, onDeskClick }) {
  ...
  const getZoneStats = (zone) => {
    // Filter desks by ID range to match rendering logic
    const zoneDesks = desks.filter(
      (d) => d.id >= zone.range[0] && d.id <= zone.range[1],
    );
    const free = zoneDesks.filter((d) => d.status === "free").length;
    const occupied = zoneDesks.filter((d) => d.status === "occupied").length;
    const away = zoneDesks.filter((d) => d.status === "away").length;
    return { free, occupied, away, total: zoneDesks.length };
  };
  ...
  return (
    <div className="desk-grid-layout">
      {zones.map((zone) => {
        ...
        // Filter desks by ID range (consistent with zone definitions)
        const zoneDesks = desks.filter(
          (d) => d.id >= zone.range[0] && d.id <= zone.range[1],
        );
        ...
        return (
          <div
            key={zone.name}
            className="glass-panel zone-section animate-fade-in"
          >
            <div className="zone-header">
              ...
              <span className="zone-badge">
                Free: {stats.free} | Occupied: {stats.occupied} | Away:{" "}
                {stats.away} | Total: {stats.total}
              </span>
            </div>
